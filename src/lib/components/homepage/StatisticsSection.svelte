    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      stats.forEach((s, i) => {
        ...
        const interval = setInterval(() => {
          step++;
          if (step >= totalSteps) { counts[i] = s.value; clearInterval(interval); }
          else { counts[i] = Math.floor((s.value * step) / totalSteps); }
        }, stepDuration);
      });
    }, { threshold: 0.3 });
    observer.observe(el);
...
<section id="stats-section" class="px-6 py-28 bg-slate-50 dark:bg-slate-900/40">
